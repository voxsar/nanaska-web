...
class TestEmailDto {
	...
	@IsOptional()
	@IsString()
	/** One of: registration | payment-receipt | enrollment-reminder | contact | newsletter */
	type?: string;
}
...
export class SettingsController {
	...
	async testEmail(@Body() body: TestEmailDto) {
		...
		await this.email.sendTestEmail(body.to, body.type ?? 'generic');
		const label = body.type ? ` (type: ${body.type})` : '';
		return { message: `Test email${label} sent to ${body.to}` };
	}
